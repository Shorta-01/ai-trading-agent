...
- **T-026** — `user-approve-action-draft.md` user-action workflow doc (JA approval ritual: /ibkr-acties Te keuren tab → per-row Goedkeuren → window.prompt → JA client-side compare → POST /approve → state transition + actor='user'; AGENTS.md §3.2 client-only enforcement re-surfaced). *Branch:* `task/T-026-reality-user-approve-action-draft`.
