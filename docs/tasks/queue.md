...
- **T-039** — `04-data-and-storage.md` architecture review doc (8 verdict-driven questions: Postgres + SQLite tests + 88-tables in 3116-LOC metadata.py + 53-migration linear chain + 47 JSON columns + Decimal-as-string + 53 hand-coded repository classes in 6617-LOC sql_repositories.py + connection pooling + single-DB deployment). *Branch:* `task/T-039-architecture-data-and-storage`.
