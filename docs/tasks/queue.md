...
- **T-041** — `06-performance-and-scale.md` architecture review doc (8 verdict-driven questions on stacked scale ceilings: single-worker uvicorn + threadpool-vs-pool mismatch + zero caching + polling everywhere + no background queue + largest modules + no APM + no CDN). *Branch:* `task/T-041-architecture-performance-and-scale`.
