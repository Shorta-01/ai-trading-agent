...
- **T-034** — `system-ibkr-submission-sweep.md` system-tick workflow doc (SubmissionSweep.tick intended for APScheduler but not wired — dominant gap; 5-stage tick body + locked one-per-tick break + 12-gate evaluation; T-019/T-020 §10.1 originating finding re-surfaced from system-tick angle). *Branch:* `task/T-034-reality-system-ibkr-submission-sweep`.
