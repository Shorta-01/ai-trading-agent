...
- **T-022** — `belgian-tax-computation.md` workflow doc (TOB per-tx compute + dividend withholding compute + action-draft TOB persistence + intent §1 compute/record split + intent §3 8-section annual report gap). *Branch:* `task/T-022-reality-belgian-tax-computation`.
