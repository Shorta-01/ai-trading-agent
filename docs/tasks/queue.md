...
- **T-030** — `user-review-decision-package-detail.md` user-action workflow doc (DP detail page with 7 locked Dutch sections; deterministic vs LLM explanation surface split; single entry point via ForecastExplanationPanel; "Maak actie" actionable-label gate). *Branch:* `task/T-030-reality-user-review-decision-package-detail`.
