...
- **T-035** — `system-ibkr-reconciliation-tick.md` system-tick workflow doc (IbkrReconciler.tick intended for APScheduler but not wired — same gap as T-034; strict Pass A → B → C ordering; 4-mode ReconcilerMode Literal; intent 15min/1h/5 triggers all missing; closes Track 1a Reality Workflows). *Branch:* `task/T-035-reality-system-ibkr-reconciliation-tick`.
