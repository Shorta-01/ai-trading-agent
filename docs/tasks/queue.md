...
- **T-021b** — `performance-review.md` functional-review carry-forward reality doc (LAST functional-review addition, closes Phase 1; screen ABSENT + backend performance.py computes only basic return-since-start not intent §8 TWR/drawdown/volatility/exposure + not API-exposed; T-044 §1 Must counterpart). *Branch:* `task/T-021b-reality-performance-review`.
