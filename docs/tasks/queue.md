...
- **T-023** — `ai-explanation-and-budget.md` workflow doc (explanation provider + lazy cache + per-provider monthly EUR budget cap + voice-rule 3-layer enforcement + Case A/B/C AI-in-forecasting re-surface). *Branch:* `task/T-023-reality-ai-explanation-and-budget`.
