...
- **T-028** — `user-acknowledge-manual-review.md` user-action workflow doc (Bevestig ritual on /admin/reconciliation: window.prompt for note → POST /acknowledge → idempotent, draft-unaffecting; "queue housekeeping not case resolution" expectation gap dominant). *Branch:* `task/T-028-reality-user-acknowledge-manual-review`.
