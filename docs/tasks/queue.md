...
- **T-042** — `07-security-observability-ops.md` architecture review doc (8 verdict-driven questions: zero auth + no CORS/CSRF + no rate limiting + plain-text env vars + append-only audit tables (the state-of-the-art piece) + unstructured logging + minimal healthcheck + no backup/DR tooling). *Branch:* `task/T-042-architecture-security-observability-ops`.
