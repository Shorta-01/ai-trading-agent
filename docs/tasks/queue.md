...
- **T-033** — `system-hourly-delta-runs.md` system-tick workflow doc (08:00-21:00 cron fires 14× daily: all 4 sub-step gates EXCLUDE hourly_delta → empty audit row + cold-start handling only; "hourly delta" name promises intra-day refresh that does NOT happen — dominant finding). *Branch:* `task/T-033-reality-system-hourly-delta-runs`.
