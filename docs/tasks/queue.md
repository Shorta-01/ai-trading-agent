...
_None. **Phase 1 audit complete** — all 66 tasks merged (T-001…T-061 + T-011b / T-011c / T-012b / T-016b / T-021b). See Done section. Phase 2 backlog inherits the 19 Track 1c Must items (T-049 §1) as initial scope._
...
- **T-012b** — `data-maturation-and-confidence-buildup.md` functional-review carry-forward reality doc (155 lines; answers 'does maturation logic exist?' — YES: sample-size confidence curve 0.40 at 60 bars → 0.95 at 252 + MINIMUM_BARS_REQUIRED=60 gate + per-predictor min-bars + /calibration/coverage 3-state + derive_confidence label chain; 5 limitations mapping to existing T-046 entries; rare 'yes it exists' functional-review addition; third of 5 functional-review additions). PR: https://github.com/Shorta-01/ai-trading-agent/pull/497 — merged 2026-05-27.
- **T-016b** — `prediction-track-record-screen.md` functional-review carry-forward reality doc (142 lines; screen spec (filters by predictor/asset/window + aggregate views + drill-downs) vs implementation status = ABSENT; backend /prediction-diary data fully present but no frontend screen (grep proof); route is list-all with no filter params; screen intended to HOST the leaderboard per predictor-lifecycle.md:56 — both absent (T-024 §10.7 + T-044 §4); Phase 1c missing-feature gap (Should, Large); fourth of 5 functional-review additions). PR: https://github.com/Shorta-01/ai-trading-agent/pull/498 — merged 2026-05-27.
- **T-021b** — `performance-review.md` functional-review carry-forward reality doc (160 lines; intent §8 screen spec (6 metric groups: TWR-vs-benchmark + drawdown + volatility/risk-budget + exposure breakdown + portfolio chart + weekly/monthly views) vs implementation status = MOST-ABSENT — screen absent (grep proof), backend `performance.py` computes 0 of 6 intent §8 metrics (only simple return-since-start, the metric intent deliberately avoids), and even the basic summary is not API-exposed (`build_portfolio_performance_summary:177` uncalled); three-layer absence; T-044 §1 Must from the functional-review angle; fifth + LAST functional-review addition — closes Phase 1). PR: https://github.com/Shorta-01/ai-trading-agent/pull/499 — merged 2026-05-27.
