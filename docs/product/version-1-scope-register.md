...
| 23 | Research source archive | V1 governance en betrouwbaarheid | Planned | Storage foundation implemented | API implemented (metadata-only) | UI implemented (metadata-only) | Implemented | This register | In progress | Metadata-only API + UI basis toegevoegd; geen upload/fetch/parsing/AI/IBKR/suggesties/orders. |
