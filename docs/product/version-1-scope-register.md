- Task 152-R3 scope note: repair-only fix for API mypy `[valid-type]` in manual `ibapi` status client factory by removing static nested class definition with runtime ibapi base variables and using isolated mypy-compatible dynamic factory construction, while preserving typed `_ManualIbapiAppProtocol` boundary, fake app injection, and real manual status-check path; no runtime widening, no sync/market-data/FX/suggestions/action drafts/orders/broker execution, no auto-connect/reconnect/persistent session manager, no schema/migrations, no `ib_insync`.
