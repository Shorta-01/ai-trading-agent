- Task 173 completed: in scope: AI foundation TS-model predictor (Slice 18 of the V1 expansion queue, fifth predictor of §21.4) — new `ai_ts_predictor` module in `packages/portfolio` with the locked input/output contract `TsModelProviderInputs` + `TsModelProviderResult` + `TsModelProviderProtocol` + `TsModelProviderUnavailable` sentinel; pure-Python `AiTsPredictor` implementing `PredictorProtocol` with provider validation (monotone quantiles p10 ≤ p50 ≤ p90, prob_gain ∈ [0, 1], confidence ∈ [0, 1]); locked blocking reasons `invalid_horizon` / `invalid_current_price` / `provider_unavailable` / `provider_error` / `invalid_quantiles` / `invalid_prob_gain` / `invalid_confidence`; provider exceptions caught at the boundary and surfaced as `provider_error`; `TsModelProviderUnavailable` injection yields a clean blocked distribution so the ensemble combiner drops the AI vote cleanly; new `ai_ts_provider` module in `apps/api` with `StubTsModelProvider` (deterministic log-returns → empirical drift + volatility → lognormal distribution; falls back to neutral on insufficient data) and `build_ts_model_provider(settings)` factory gated on `ai_ts_predictor_enabled` + `ai_ts_predictor_provider_code="stub"`; real-provider codes (anthropic / timesfm / chronos / lag-llama) return `TsModelProviderUnavailable("real_client_not_implemented")` for V1; new settings `ai_ts_predictor_enabled` (False default) + `ai_ts_predictor_real_client_enabled` (False default) + `ai_ts_predictor_provider_code` (default "stub"). Out of scope (confirmed not added): real TimesFM / Chronos / Lag-Llama HTTP clients (a future widening adds those behind the existing factory boundary), persisting AI provider responses to a separate audit table (the existing Prediction Diary tracks per-predictor accuracy via the ensemble combiner), per-symbol prompt customisation (the inputs already carry symbol + sector for future providers), embedding-based research retrieval (the AI explanation layer from Slice 10 handles narrative output; the TS predictor handles numeric forecasts only), batched-inference optimisation (the V1 path is per-asset). The AI predictor is drop-in interchangeable with GBM, Momentum, Mean-Reversion, and QVM through `PredictorProtocol`.
- Task 172 completed: in scope: Universe scan orchestrator + locked registry + routes (Slice 17 of the V1 expansion queue) — new `universe_registry` module listing the V1 locked universe (BEL20 20, AEX 25, CAC40 40, DAX40 40, SP100 100, NASDAQ100-extras ~65) with `UniverseEntry(symbol, eodhd_symbol, index_code, sector)`; `locked_universe()` deduplicates by EODHD symbol so cross-listed tickers (HON, ASML, others) only count once; `universe_by_index(index_code)` helper; storage migration `0039_universe_scan_runs` adds the audit table (locked status set, locked triggered_by set, non-negative counts, safety booleans hard-False); `UniverseScanRunRecord` dataclass with `__post_init__` invariants; `SqlAlchemyUniverseScanRunRepository` (save / update / latest / list); new `universe_scan_sync` orchestrator with `scan_universe(...)` that iterates the locked universe (capped at `universe_scan_max_tickers_per_run`), per ticker fetches EODHD fundamentals + EOD bars, derives 6m/12m returns from bars when fundamentals omit them, persists an `AssetFundamentalsSnapshotRecord`, and reports per-ticker failures without aborting; scan_run row persisted as `running` first then updated to terminal state; companion `build_universe_fundamentals(snapshots)` helper translates snapshots into `UniverseFundamentals` for the QVM predictor; two new routes `POST /universe/scan/run` (gated on `universe_scan_sync_enabled` + EODHD enabled + EODHD key + writable storage) and `GET /universe/scan/runs/latest`; new settings `universe_scan_sync_enabled` (False default), `universe_scan_max_tickers_per_run` (50 default), `universe_scan_history_lookback_days` (400 default). Out of scope (confirmed not added): the full ~5 000-ticker universe (a later widening expands the registry; the V1 doctrine §21.6 anticipated this), persisting per-asset QVM rankings to a separate `universe_scan_candidates` table (V1 keeps the ranking computed in-memory), wiring the universe scan into the forecast sync (Slice 19+ orchestrates the full ensemble), scheduler-driven universe scans (Slice 21 wires the morning chain), automatic universe-set expansion based on market-cap thresholds, sector-neutral z-scoring.
- Task 171 completed: in scope: QVM factor predictor + EODHD fundamentals fetcher + fundamentals storage (Slice 16 of the V1 expansion queue) — storage migration `0038_asset_fundamentals_snapshots` (UNIQUE on `(eodhd_symbol, fetched_at)`, safety booleans hard-False); `AssetFundamentalsSnapshotRecord` dataclass; `SqlAlchemyAssetFundamentalsSnapshotRepository` (save + latest-by-symbol + group-by-symbol latest universe); extended `EodhdClient` with `fetch_fundamentals(eodhd_symbol)` hitting `/fundamentals/{SYMBOL.EX}` + `EodhdFundamentals` dataclass + `_parse_fundamentals` reading General / Highlights / Valuation / Technicals; locale-aware `_percent_or_none` treats ratios ≤ 1.5 as fractions and multiplies by 100; pure-Python `QvmFactorPredictor` in `packages/portfolio` implementing `PredictorProtocol`: Quality = z-score of (ROIC + gross_margin); Value = z-score of inverted clipped P/E + P/B + EV/EBITDA (P/E [1, 80], P/B [0.1, 25], EV/EBITDA [1, 60]); Momentum = z-score of (6m + 12m return); composite is soft-clipped at ±2 to [-1, +1] then mapped to ±25% annualised drift, with trailing 6-month volatility scaling the lognormal distribution; uniform universe spread yields a z-score of 0 (no cross-sectional signal); locked blocking reasons `invalid_horizon` / `invalid_current_price` / `insufficient_history` (< 130 bars) / `insufficient_universe` (< 5 entries) / `symbol_not_in_universe` / `insufficient_factors` / `flat_history`; `UniverseFundamentals(entries=tuple[FundamentalsEntry, ...])` is injected directly in this slice. Out of scope (confirmed not added): wiring the universe scan into the orchestrator (Slice 17), persisting per-asset QVM scores to a dedicated table (kept in-memory in the snapshot), AI predictor (Slice 18), Diary plumbing for per-predictor contributions (Slice 15 set up `EnsembleContribution`; persisting it lands later), inverse-variance reweighting, sector-neutral z-scores (V1 keeps a single universe-wide cross-section).
- Task 170 completed: in scope: Mean-reversion predictor + deterministic ensemble combiner (Slice 15 of the V1 expansion queue) — new `MeanReversionPredictor` (pure Python) combining RSI(14) with Wilder smoothing, Bollinger z-score over a 20-day SMA (capped at ±3), and Hurst exponent estimated via a multi-window rescaled-range fit across chunk sizes 10/20/50/100 over the trailing 100-day window; composite pull factor = average of RSI + Bollinger strength multiplied by Hurst confidence (1 - 2H clamped to [0,1]) so trending series collapse the pull toward zero; projected price = `current + pull × (SMA_20 - current)` with distribution width scaled by trailing 6-month volatility; locked blocking reasons `invalid_horizon` / `invalid_current_price` / `insufficient_history` (< 130 bars) / `flat_history` (no recent volatility); confidence heuristic multiplied by Hurst confidence so trending series also cap confidence. New `ensemble_combiner` module with `compute_ensemble_forecast(predictors, inputs, weights)`: runs every predictor, drops blocked results, combines surviving distributions via weighted averages of p10/p50/p90, expected_return_pct, prob_gain/loss, confidence; direction is re-derived from combined expected return using the same thresholds; confidence is multiplied by an `agreement_factor` (1.0 on full agreement, 0.6 on partial mix, 0.4 on positive-vs-negative split); default equal-weight; custom `weights: dict[str, Decimal]` (model_code → weight) normalised over surviving predictors; missing entries default to 1.0; non-positive weights fall back to 1.0; zero total weight yields a blocked ensemble with reason `zero_total_weight`; empty predictor list yields `no_predictors`; all-blocked yields `all_predictors_blocked` with the blocked codes preserved in `EnsembleResult.blocked_model_codes`; returns `EnsembleResult(forecast, contributions, blocked_model_codes)` with per-predictor `EnsembleContribution(model_code, model_version, weight_raw, weight_normalised, prediction)` for downstream Diary tracking. Out of scope (confirmed not added): wiring the ensemble into `forecast_sync` (Slice 17), QVM factor predictor (Slice 16), AI foundation TS predictor (Slice 18), inverse-variance reweighting (deferred until Diary track-record exists), persisting per-predictor contributions to a storage table (the data structures are ready; the storage row is a later slice), per-asset weight overrides keyed by sector or regime.
- Task 169 completed: in scope: `PredictorProtocol` + Momentum predictor (Slice 14 of the V1 expansion queue) — new `predictor_protocol` module defining `PredictorInputs(historical_bars, current_price, horizon_trading_days, asset_metadata)` and `PredictionDistribution` (locked direction set `strong_up`/`slight_up`/`flat`/`slight_down`/`strong_down`, locked status set `ready`/`blocked`, post-init invariants on blank model_code/version + non-positive horizon + unknown direction/status); `PredictorProtocol` Protocol class; `GbmPredictor` adapter that wraps the existing `compute_baseline_forecast` so the math is unchanged but exposed via the protocol (GBM "neutral" → protocol "flat"; blocked forecasts default direction to "flat"); new `MomentumPredictor` (pure Python, no SciPy) combining 12-1 momentum (12-month log-return excluding most-recent month, normalised by `log(1.25)`) with time-series momentum (6-month log-return ÷ (6-month SD × √horizon)) into a composite score capped at [-1, +1] then mapped to ±25% annualised drift, with trailing 6-month volatility scaling the lognormal distribution; deterministic blocking reasons `invalid_horizon` / `invalid_current_price` / `insufficient_history` (< 250 bars) / `flat_history` (no recent volatility); same direction thresholds and confidence heuristic as GBM. Out of scope (confirmed not added): the ensemble combiner (Slice 15), wiring the new predictors into `forecast_sync` (Slice 17), mean-reversion / factor / AI predictors (Slices 15, 16, 18), per-predictor Diary track-record (Slice 15 plumbs the boundary; later slices accumulate the history), inverse-variance reweighting (deferred until Diary has data).
- Task 168 completed: in scope: doctrine relock + APScheduler skeleton (Slice 13 of the V1 expansion queue) — `version-1-product-experience-locks.md §21.1` relocks applied in code: removed `account_mode_mismatch` from `run_dry_run_safety_checks`; removed `version1_paper_only` + `account_mode_mismatch` from `ibkr_sync_readiness`; removed `ibkr_expected_environment` + `ibkr_sync_account_mode` gates from `build_real_order_submission_client`. New route `GET /ibkr/account/mode` reports mode (PAPER / LIVE / UNKNOWN) without authorising any order. Storage migration `0037_scheduler_runs` adds the `scheduler_runs` table (locked status set, locked triggered_by set, safety booleans hard-False) + `SchedulerRunRecord` + `SqlAlchemySchedulerRunRepository` (save / update / latest / list). New `scheduler` module integrates `apscheduler.BackgroundScheduler` into the FastAPI lifespan behind `scheduler_enabled` (default False); daily_briefing job is a skeleton callable for V1 — Slice 21 wires the full morning chain. `run_daily_briefing_job(...)` captures every fire as an audit row with running → succeeded/failed transitions; persistence failures never crash the scheduler. Two new routes `GET /scheduler/jobs` + `GET /scheduler/runs/latest`. New settings `scheduler_enabled` (False default) + `scheduler_timezone` (Europe/Brussels) + `scheduler_daily_briefing_cron` (`30 6 * * *`). Health endpoint loses the hard-coded `mode: "paper-only"` field; main root endpoint same. Web: PAPER / LIVE badge in the Portefeuille header (info-tone paper, warning-tone live, muted unknown) + Scheduler panel with status / cron / next-fire / last-run. APScheduler added as apps/api runtime dep with mypy `ignore_missing_imports`. Out of scope (confirmed not added): wiring the morning-chain orchestrator into the scheduler job (deferred to Slice 21), real-money submission throttles (manual approval is the only remaining safety surface), removing or rewriting the existing `paper_only_mode` / `ibkr_expected_environment` settings (kept as informational config so existing tests + persisted records keep working), changes to the action-draft state machine, any new predictor.

## V1 expansion (post-Slice 12, locked via §21 in version-1-product-experience-locks.md)

Following an owner brainstorm after Slice 12, V1 scope expands with 10 more slices (13–22) and three doctrine relocks. The expansion is **still V1** — not a V2 release. Relocks: (a) account-mode is reported, not gated (paper/live decided by the IBKR account); (b) AI participates as one ensemble predictor (not an authoritative number originator, not paraphrase-only); (c) order vocabulary extends to the full IBKR set (MKT, STP, STP LMT, TRAIL, TRAIL LMT, bracket, conditional). Manual approval gate, Decimal-only math, locked Dutch action labels, Decision Package evidence chain, Prediction Diary outcome tracking, and `safe_for_*=False`-on-persisted-records all stay. Sizing moves from fixed-buy-value to fractional Kelly + risk parity. Universe scan adds ~5 000 EU + US tickers ranked by QVM factors. Scheduler (APScheduler in-process) drives a 06:30 Brussels cron so the 07:00 briefing is pre-computed.

Queue: Slice 13 (doctrine relock + scheduler skeleton) → 14 (PredictorProtocol + momentum) → 15 (mean-reversion + ensemble combiner) → 16 (QVM factor predictor) → 17 (universe scan) → 18 (AI foundation TS predictor) → 19 (fractional Kelly + risk parity) → 20 (full IBKR order vocabulary) → 21 (scheduler-driven morning chain) → 22 (release readiness + acceptance test).

---

- Task 167 completed: in scope: deterministic daily briefing + alert digest end-to-end (Slice 12) — storage migration `0036_daily_briefings` adding `daily_briefings` (UNIQUE per `briefing_date`, hard-False safety booleans `safe_for_action_drafts` + `safe_for_orders`) and `briefing_alerts` (append-only, hard-False safety booleans); `DailyBriefingRecord` + `BriefingAlertRecord` dataclasses with non-negative-counts + severity invariants; `SqlAlchemyDailyBriefingRepository` (upsert-by-date + latest + alert save/list/delete); pure-Python `daily_briefing` module in `packages/portfolio` with typed `BriefingInputs` and `compute_daily_briefing(...)`; deterministic alert kinds (`new_suggestion`, `new_decision_package`, `new_action_draft`, `diary_outcome_closed`, `critical_draft_event`, `fx_stale`) with locked severity mapping; `daily_briefing_sync.generate_daily_briefing(...)` orchestrator in `apps/api`; two new routes `POST /briefings/daily/compute` + `GET /briefings/daily/latest`; new settings `daily_briefing_sync_enabled` + `daily_briefing_lookback_hours`; Portefeuille Dagbriefing panel with generate / refresh buttons and coloured alert list. Out of scope (confirmed not added): AI authoring of briefings, automatic scheduling (the operator triggers the briefing), e-mail / push delivery (renders in-page only), acknowledgement workflow (alerts have an `acknowledged_at` column but no UI to set it yet), per-user briefings (the briefing is global to the operator's portfolio in V1), broker execution, lifting any safety boolean.
- Task 166 completed: in scope: Belgian tax module wired into the action draft Orderimpact (Slice 11) — new pure-Python `belgian_tax` module in `packages/portfolio` with locked TOB rates (0.12% bonds, 0.35% standard stock/distributing ETFs, 1.32% accumulating ETFs/SICAV redemptions) and per-transaction caps (€1 300 / €1 600 / €4 000) + 30% dividend roerende voorheffing; `TobSecurityClass` StrEnum; `compute_tob(*, transaction_value, security_class)` returns HALF_UP-rounded cents with the cap, zero returns zero, negative raises `ValueError`, non-Decimal raises `TypeError`; `compute_dividend_withholding(*, gross_dividend)` same contract at 30%; extended `Orderimpact` dataclass with `estimated_belgian_tob` + `belgian_tob_security_class`; `compute_orderimpact(..., belgian_tob_security_class=TobSecurityClass.STANDARD_STOCK)` threads the TOB through; storage migration `0035_action_draft_belgian_tob` adds two nullable columns to `asset_action_drafts`; `AssetActionDraftRecord` gains both fields with non-negative invariant; `action_draft_sync` orchestrator persists the TOB; serializer surfaces it on the latest + per-draft endpoints; Portefeuille Action drafts table renders a "TOB (BE)" column with the security-class on hover. Out of scope (confirmed not added): real-time TOB lookup from a broker (the rate is derived from a locked table), country-of-residence determination (V1 assumes Belgian resident), per-asset security-class classification (V1 defaults every draft to `standard_stock` because the locked action-draft scope is listed shares), capital-gains tax (Belgium has no general CGT on listed shares for individuals; future complement for accumulating funds, see Reynders-Cooreman law, is deferred), withholding on coupon income, broker execution, AI authoring of tax math, lifting any safety boolean via tax preview.
- Task 165 completed: in scope: AI explanation layer (RAG read-only) end-to-end (Slice 10) — new storage migration `0034_decision_package_explanations` adding `decision_package_explanations` table (UNIQUE on `(decision_package_id, decision_package_content_hash)`, hard-False safety booleans `safe_for_self_learning` + `safe_for_action_drafts` + `safe_for_orders`) and `explanation_evidence_ledger` (append-only, hard-False safety booleans `safe_for_self_learning` + `safe_for_model_retraining`); `DecisionPackageExplanationRecord` + `ExplanationEvidenceLedgerRecord` dataclasses with safety-boolean-True invariants; `SqlAlchemyDecisionPackageExplanationRepository` (save + latest-by-package + by-version + ledger save + ledger list); pure-Python `ai_explanation_guards.validate_explanation_output(...)` in `packages/portfolio` with locked V1 doctrine (no hallucinated numbers via locale-aware tokenizer, locked Dutch disclaimer required, max-length + empty checks); deterministic status outcomes `generated` / `blocked` with explicit reasons `empty_output` / `output_too_long` / `disclaimer_missing` / `hallucinated_numbers`; new `ai_explanation_provider` in `apps/api` with `StubExplanationProvider` (deterministic paraphrase of existing Decision Package fields, no network, no AI runtime) + `ExplanationProviderProtocol` boundary + `build_explanation_provider(settings)` factory returning `ExplanationProviderUnavailable(reason="real_client_not_implemented")` for non-stub providers; `ai_explanation_sync.generate_explanation(...)` orchestrator (canonical input JSON + hash + provider call + validator + persistence + one ledger entry per evidence source); two new routes `POST /decision-packages/{decision_package_id}/explanation` + `GET /decision-packages/{decision_package_id}/explanation`; new settings `ai_explanation_enabled` / `ai_explanation_real_client_enabled` / `ai_explanation_provider_code` / `ai_explanation_max_output_chars`; Portefeuille Decision Package card AI uitleg subsection (generate / read buttons + provider/model footer + blocked-reason warning + full Dutch text). Out of scope (confirmed not added): real Anthropic / OpenAI / other AI client runtime (the factory returns Unavailable for non-stub provider codes), AI authoring of action labels (still the deterministic translator from Slice 4), AI overriding any safety gate, automatic explanation refresh scheduler, batch generation across the portfolio, citation lookup or retrieval-rerank steps, broker execution, fake or invented numeric outputs, lifting safety booleans.
- Task 164 completed: in scope: Research Desk runtime wired into the Decision Package evidence chain (Slice 9) — migration `0033_decision_package_research_evidence` adding five new nullable columns (`research_evidence_count` default 0 + `research_credibility_summary` + `research_freshness_status` + `research_blocking_reason` + `research_snippet_nl`), extended `AssetDecisionPackageRecord` with non-negative count invariant, pure-Python `research_evidence_summary` module in `packages/portfolio` (`ResearchEvidenceInputs`, `ResearchEvidenceSummary`, `summarize_research_for_asset(sources, *, now)` with deterministic credibility/freshness aggregation and locked blocking reasons `prompt_injection_high_risk` + `credibility_rejected`), new `build_research_summary_by_symbol(symbols, *, research_repo, now)` helper in `decision_package_sync`, threaded `research_summary_by_symbol` kwarg into `sync_decision_packages(...)`, updated content_hash to include the four scalar research fields (so research changes produce new versions), updated `serialize_decision_package_for_response` to surface the five research fields, route handler `POST /decision-packages/compute` now builds the summary before invoking the orchestrator, Portefeuille Decision Package card renders a Research subsection (count + credibility + freshness + snippet + blocking warning). Out of scope (confirmed not added): AI authoring of research, AI explanation runtime (next slice), automatic linking of new uploads to assets, attaching research items to a specific suggestion (the link is per asset symbol via the existing `research_source_asset_links`), new web upload flow, fake credibility/freshness values, lifting any block via research, automatic broker execution, new safety-flag promotion.
- Task 163 completed: in scope: reconciliation + Prediction Diary end-to-end (Slice 8) — new `prediction_diary_entries` table (migration 0032, UNIQUE on `suggestion_id`, safety booleans hard-False), `PredictionDiaryEntryRecord` dataclass + `SqlAlchemyPredictionDiaryRepository` (upsert-by-suggestion-id + list), pure-Python `prediction_diary_eval` with deterministic outcome labels (`right` / `wrong` / `early` / `inconclusive` / `no_data`) per horizon, `reconciliation_sync.reconcile_submissions(...)` matching submissions against IBKR open-orders + executions and auto-advancing FILLED/CANCELLED → RECONCILED via the locked state machine (critical-severity event per transition + weighted avg fill price), `prediction_diary_sync.evaluate_prediction_diary(...)` building one diary row per suggestion using the linked forecast + persisted EOD bars with 7-day weekend walk-back, three new routes (`POST /action-drafts/reconcile`, `POST /prediction-diary/evaluate`, `GET /prediction-diary`), new settings `reconciliation_sync_enabled` / `prediction_diary_sync_enabled` / `prediction_diary_inconclusive_tolerance_pct`. Out of scope (confirmed not added): real-money mode (paper-only locked), auto-reconciliation scheduler/background loop, automatic re-submission on cancel, partial-fill state expansion beyond still-working, self-learning loops (`safe_for_self_learning` / `safe_for_model_retraining` remain hard-False), Research Desk runtime (next slice), AI-originated outcome labels, watchlist diary surface, fake reconciliation results, fake outcome labels.
- Task 162 completed: in scope: user-approved IBKR paper submission with locked state-machine handshake (Slice 7) — `ActionDraftState` enum + `ALLOWED_TRANSITIONS` graph + `require_transition_allowed` in `packages/portfolio`, new `asset_action_draft_submissions` + `asset_action_draft_events` tables (migration 0031), `AssetActionDraftSubmissionRecord` + `AssetActionDraftEventRecord` + matching repositories, real `IbapiOrderSubmissionClient` paper-only placeOrder helper (LMT/DAY/whole-share STK with dependency-isolated ibapi import + injectable app for tests), `build_real_order_submission_client` factory gated on submission_enabled + real_client_enabled + paper account_mode + paper expected_environment + host/port/client-id, `action_draft_submission` orchestrator (`approve_action_draft` + `submit_action_draft_to_paper`), three new routes (`POST /action-drafts/{id}/approve`, `POST /action-drafts/{id}/submit-to-ibkr-paper`, `GET /action-drafts/{id}/status`), critical-severity audit events per state transition. Out of scope (confirmed not added): real-money mode (paper-only locked), auto-submission, batch approval, scheduled re-submission, PATCH editable endpoint (deferred — would need re-sizing + dry-run revalidation; the existing draft + new approval flow suffices for V1), cancel-against-IBKR (next slice with reconciliation), reconciliation engine (next slice), watchlist submission surface, Prediction Diary (next slice), AI-originated submissions, fake submission results, fake order IDs.
- Task 161 completed: in scope: action draft skeleton + Orderimpact + dry-run end-to-end (Slice 6) — `action_draft_safety` math in `packages/portfolio` (sizing + Orderimpact + deterministic dry-run failure codes), new `asset_action_drafts` table (migration 0030), `AssetActionDraftRecord` (with LMT/DAY/BUY-SELL/positive quantity-and-price + safety-flag-False invariants), `SqlAlchemyAssetActionDraftRepository`, `action_draft_sync.generate_action_drafts(...)` orchestrator filtering to the four actionable locked labels, three new routes (`POST /action-drafts/compute`, `GET /action-drafts/latest`, `GET /action-drafts/{draft_id}`), Portefeuille Action drafts table with Orderimpact preview + dry-run badge. Out of scope (confirmed not added): order submission, broker action, IBKR reply handshake, action-draft editing API (the PATCH lives in Slice 7), final-confirmation flow, batch approval, alerts on state transition, AI-originated drafts, fake submission results, watchlist action drafts surface.
- Task 160 completed: in scope: Decision Package foundation end-to-end (Slice 5) — immutable, content-hashed `asset_decision_packages` table (migration 0029), `AssetDecisionPackageRecord` with denormalised evidence chain, insert-only `SqlAlchemyAssetDecisionPackageRepository`, `decision_package_sync.sync_decision_packages(...)` orchestrator with SHA-256 content hash, three new routes (`POST /decision-packages/compute`, `GET /decision-packages/latest`, `GET /decision-packages/{conid}/latest`), Portefeuille Decision Packages drilldown section. Out of scope (confirmed not added): action drafts, dry-run, `Orderimpact`, orders, broker submission, AI authoring of packages, evidence-ledger linking (research evidence not wired yet), Decision Package mutation/versioning UI, watchlist Decision Packages surface (Portefeuille only this slice), fake evidence values.
- Task 159 completed: in scope: deterministic Dutch label translator + Suggestions foundation end-to-end (Slice 4) — `baseline_label_translator` pure-Python rule engine over the locked label set, new `asset_suggestions` table (migration 0028) + `AssetSuggestionRecord` + repository, `suggestion_sync` orchestrator, `POST /suggestions/compute` + `GET /suggestions/latest` routes, Portefeuille "Actie" badge column. Out of scope (confirmed not added): AI runtime, action drafts, orders, broker execution, dry-run, "Orderimpact", scheduler/background fetch, batch approval, suggestion ledger/history UI, watchlist suggestions UI surface (Portefeuille only this slice), fake decisions, AI-originated labels.
- Task 158 completed: in scope: baseline forecast engine end-to-end (Slice 3, V1.1 stage) — `compute_baseline_forecast` lognormal GBM math, two new storage tables (migration 0027), `EodhdClient.fetch_eod_bars`, `forecast_sync` orchestrator, `POST /forecasts/compute` + `GET /forecasts/latest` routes, Portefeuille "Verwachte richting" badge column. Out of scope (confirmed not added): AI runtime, suggestions, action drafts, orders, broker execution, scheduler/background fetch, multi-asset/portfolio-level forecasts, ensemble or challenger models, web UI changes beyond the one badge column, fake forecast values.
- Task 157 completed: in scope: real market-data + FX provider via EODHD end-to-end (Slice 2) — `EodhdClient` HTTP, sync orchestrator with IBKR→EODHD exchange mapping and FX-pair derivation, factory and `POST /market-data/sync` route, persistence into existing market-data + FX tables. Out of scope (confirmed not added): suggestions, action drafts, orders, broker execution, scheduler/background fetch, schema/migration changes, web UI changes, fake market/FX data.
- Task 156 completed: in scope: real IBKR paper read-only sync runtime (Slice 1) — real `ibapi`-backed adapter, safety-gated factory, route wiring with explicit `close()`. Out of scope (confirmed not added): orders, suggestions, action drafts, market-data runtime, FX runtime, auto-connect/reconnect/persistent session manager, schema/migration changes, fake broker data.
...
- Task 152-R4 scope note: repair-only API pytest expectation alignment after Task 152-R3 merged-red CI; stale fake-client manual status-check tests now use explicit fake-client-ready settings that enable all required gates before expecting connect/check/disconnect path, default-blocked tests remain unchanged, and no-secret tests now reject exact raw configured values while allowing safe diagnostic code names (`missing_host`, `missing_port`, `missing_client_id`); no runtime widening, no sync/market-data/FX/suggestions/action drafts/orders/broker execution, no auto-connect/reconnect/persistent session manager, no schema/migrations, no `ib_insync`.
- Task 152-R2 scope note: Ruff B009 repair-only for manual `ibapi` status client internals (replace constant-attribute `getattr(...)` with Ruff-compliant direct attribute access or narrow equivalent) while preserving typed protocol/factory boundary and real manual status-check path; runtime remains disabled-by-default, readiness never connects, no sync/market-data/FX/suggestions/action drafts/orders/broker execution additions, no auto-connect/reconnect/persistent session manager, no schema/migrations, no `ib_insync`.
- Task 152-R scope note: typing-boundary repair only for manual IBKR status client internals; keeps real manual status-check path, no runtime widening, no sync/market-data/FX/orders/broker execution additions, runtime still disabled by default.
