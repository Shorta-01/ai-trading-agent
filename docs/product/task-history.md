- **Task 152-R3 (repair-only, completed):** Repaired merged-red CI after Task 152-R2 in `api` job at step `mypy src`; failure was mypy `[valid-type]` from using runtime variables (`ewrapper_type`, `eclient_type`) as base classes in a nested class definition inside the manual `ibapi` app factory. Replaced static nested class with isolated mypy-compatible dynamic factory (`type(...)`) and kept `_ManualIbapiAppProtocol` boundary plus fake app injection and real manual status-check capability intact. No runtime widening: runtime disabled by default, readiness never connects, no sync/market-data/FX/suggestions/action drafts/orders/broker execution, no auto-connect/reconnect/persistent session manager, no schema/migrations, no `ib_insync`, no API/web behavior changes beyond existing manual status endpoint/readiness behavior.
