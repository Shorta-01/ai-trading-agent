...
- **Completed/foundation:** metadata-archief, veilige upload, upload-UI, TXT/MD/CSV extractie.
- **Still missing:** PDF/DOCX/XLSX/PPTX extractie, eventuele OCR, extractie-trigger/status-UX verdieping, classificatie-runtime, prompt-injection runtime scan, source credibility scoring runtime, evidence extraction, multi-year vergelijking-runtime, URL fetching + veilige snapshotting, asset detectie + watchlist proposal flow.
- **Next likely tasks:** UI extractie-trigger + extracted-text status; daarna classificatie foundation.
...
- **Next likely tasks:** kalender-provider en freshness checker design.
...
- **Next likely tasks:** eerste model-slice + teststrategie.
- **Safety notes:** Elke financiële berekening getest.

## 9) AI Event Intelligence
