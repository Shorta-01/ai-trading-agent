...
- Slice 13 — Task 168: doctrine relock + scheduler skeleton. Relax `paper_only_mode` + `ibkr_expected_environment` to "report mode, never block"; dashboard `PAPER` / `LIVE` badge; APScheduler in-process behind `scheduler_enabled` (default False); `scheduler_runs` audit table; routes `GET /scheduler/jobs` + `GET /scheduler/runs/latest`. No new predictors.
