...
- Slice 14 — Task 169: `PredictorProtocol` in `packages/portfolio` (input bars, output `PredictionDistribution{p10/p50/p90, prob_gain, direction, confidence, model_code}`); migrate the existing GBM to the protocol; add Momentum predictor (12-1 + time-series).
- Slice 15 — Task 170: Mean-reversion predictor (RSI / Bollinger / Hurst / z-score) + deterministic `compute_ensemble_forecast(predictors, bars, weights)` combiner (equal-weight at launch, evolves to inverse-variance). Per-predictor contribution stored; Diary tracks per-model accuracy.
- Slice 16 — Task 171: QVM factor predictor (Quality: ROIC + gross margins; Value: P/E + P/B + EV/EBITDA; Momentum: 6m + 12m). New EODHD fundamentals fetcher.
