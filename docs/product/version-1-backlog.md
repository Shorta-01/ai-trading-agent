...
**V1 is feature-complete.** Slices 13–22 of the V1 expansion queue
are all merged. The morning chain runs end-to-end (06:30
Europe/Brussels), the manual approval gate remains the only
order-authorisation surface, every persisted record keeps
...
- Slice 13 — Task 168 ✅ **completed**: doctrine relock + scheduler skeleton. Dropped `account_mode_mismatch` dry-run failure, the `version1_paper_only` readiness blocker, and the order-submission factory mode gates per §21.1. Added `GET /ibkr/account/mode` for the dashboard badge. New `scheduler_runs` table + record + repo. APScheduler `BackgroundScheduler` wired into FastAPI lifespan behind `scheduler_enabled` (default False). Routes `GET /scheduler/jobs` + `GET /scheduler/runs/latest`. Web: PAPER / LIVE badge + Scheduler panel.
- Slice 14 — Task 169 ✅ **completed**: `PredictorProtocol` + GBM adapter + Momentum predictor (12-1 + time-series momentum). Pure Python, no orchestrator change.
- Slice 15 — Task 170 ✅ **completed**: Mean-reversion predictor (RSI + Bollinger + Hurst) + deterministic ensemble combiner with weighted averages + agreement-factor confidence. Per-predictor `EnsembleContribution` breakdown ready for downstream Diary tracking.
- Slice 16 — Task 171 ✅ **completed**: QVM factor predictor + EODHD fundamentals fetcher + `asset_fundamentals_snapshots` storage. Pure-Python predictor implementing `PredictorProtocol` with cross-sectional QVM z-scores; universe still injected as a fixture (Slice 17 wires the scan).
- Slice 17 — Task 172 ✅ **completed**: Locked universe registry (Bel20 + AEX + CAC40 + DAX40 + S&P 100 + NASDAQ100 extras, ~325 tickers, dedup-by-symbol; full ~5 000-ticker expansion is a later widening), `universe_scan_runs` audit table + record + repository, `scan_universe(...)` orchestrator that fetches fundamentals + bars and persists per-ticker snapshots, two new routes `POST /universe/scan/run` + `GET /universe/scan/runs/latest`. Universe-fundamentals helper translates persisted snapshots into the QVM predictor's input shape.
- Slice 18 — Task 173 ✅ **completed**: AI foundation TS-model predictor behind a stub-or-real provider boundary. Pure-Python `AiTsPredictor` in `packages/portfolio` validates the provider output + degrades gracefully when the provider is unavailable; `StubTsModelProvider` in `apps/api` ships as the deterministic default. Factory returns `real_client_not_implemented` for non-stub provider codes until a future widening.
- Slice 19 — Task 174 ✅ **completed**: Pure-Python `kelly_sizing` module with asymmetric Kelly formula, half-Kelly default, per-asset (5%) and per-sector (30%) caps. `DraftSourceContext` extended with ensemble distribution + sector exposure; `Kopen` BUY path uses Kelly when ensemble inputs present, falls back to default-buy-value otherwise.
- Slice 20 — Task 175 ✅ **completed**: Full IBKR order vocabulary (§21.3 lock) — `AssetActionDraftRecord` extended with `stop_price`, `trail_amount`, `trail_percent`, `bracket_take_profit_limit_price`, `bracket_stop_loss_price`; `LOCKED_ORDER_TYPES = {LMT, MKT, STP, STP_LMT, TRAIL, TRAIL_LMT, BRACKET}`; per-type `__post_init__` invariants. Migration `0040_action_draft_order_vocabulary` adds the five new nullable Decimal columns. `run_dry_run_safety_checks(...)` gains per-order-type validation with stable failure codes (`unsupported_order_type`, `stp_missing_stop_price`, `stp_lmt_missing_stop_or_limit`, `trail_missing_trail_value`, `trail_amount_and_percent_set`, `trail_lmt_missing_limit_price`, `bracket_missing_take_profit`, `bracket_missing_stop_loss`, `bracket_take_profit_below_limit`, `bracket_stop_loss_above_limit`, `bracket_inverted_for_sell`). `IbapiOrderSubmissionClient` gets a new `build_contract_and_orders(...)` builder that maps each order type to ibapi `Order` flags (`orderType="LMT|MKT|STP|STP LMT|TRAIL|TRAIL LIMIT"`, `auxPrice`, `trailingPercent`, `lmtPrice`); BRACKET emits the parent (LMT, transmit=False) + take-profit child (opposite-side LMT, transmit=False) + stop-loss child (opposite-side STP, transmit=True) with `parentId` patched to the parent's order_id; the submission flow places three orders on the next-valid id + 1 + 2 and records the parent's id/perm_id on the result. `OrderSubmissionInputs` extended with the new fields; `submit_action_draft_to_paper(...)` threads them from the draft. No new routes; manual approval gate stays; safety booleans hard-False on every persisted record.
- Slice 21 — Task 176 ✅ **completed**: Scheduler-driven 07:00 morning chain — new `morning_chain` orchestrator composes the six-leg V1 chain (market-data sync → ensemble forecast → suggestions → Decision Packages → action drafts → daily briefing) into a single sequenced runner with `MorningChainLegOutcome(leg_name, status, failure_code, detail_nl)` and `MorningChainResult` dataclasses, stable per-leg names locked in `MORNING_CHAIN_LEG_NAMES`, and a `MorningChainFailed` exception for the scheduler-job wrapper. `run_morning_chain(legs)` iterates in order, treats `skipped` as "continue" and `failed` as "stop"; raised exceptions translate to a `leg_callable_raised` failure on the audit chain. `build_scheduler_chain_callable(legs_factory)` adapts the chain into the no-arg callable consumed by `run_daily_briefing_job(...)` so the scheduler's audit row captures success/failure transparently. `build_default_morning_chain_legs(settings)` produces the six production legs; each respects its own `<leg>_sync_enabled` flag (`market_data_sync_enabled`, `forecast_sync_enabled`, `suggestions_sync_enabled`, `decision_packages_sync_enabled`, `action_drafts_sync_enabled`, `daily_briefing_sync_enabled`) so the chain can be opted into incrementally. `install_default_jobs(...)` now defaults the registered job's callable to the morning-chain runner — the operator no longer needs to inject one. New route `POST /scheduler/runs/morning-chain` exercises the chain on demand: gated on `scheduler_enabled` + writable storage; persists a `SchedulerRunRecord` with `triggered_by="manual"`; returns the full per-leg outcome plus the run id; failed legs surface in the audit row's `error_text` as `{failed_leg}:{failure_code}`. Manual approval gate stays; safety booleans hard-False on every persisted record and every response. The full briefing-source distinction (portfolio / watchlist / universe_scan_candidate) is deferred to a post-V1 widening — the chain already feeds the suggestion + briefing pipeline so universe-scan candidates flow through the existing suggestion-row path.
- Slice 22 — Task 177 ✅ **completed**: V1 release readiness — pure-Python `release_readiness` module in `apps/api` aggregates the storage / EODHD / IBKR / scheduler / morning-chain leg / audit-pad settings into a Dutch scorecard with 15 stable blocker codes (`storage_not_configured`, `storage_not_writable`, `eodhd_not_configured`, `eodhd_api_key_missing`, `ibkr_not_enabled`, `ibkr_sync_not_enabled`, `scheduler_disabled`, `market_data_sync_disabled`, `forecast_sync_disabled`, `suggestions_sync_disabled`, `decision_packages_sync_disabled`, `action_drafts_sync_disabled`, `daily_briefing_sync_disabled`, `reconciliation_sync_disabled`, `prediction_diary_sync_disabled`); `compute_release_readiness(settings) -> ReleaseReadinessReport(status, summary_nl, help_nl, blockers, checks)` with `STATUS_READY` / `STATUS_BLOCKED` and per-check `ReadinessCheck(code, passed, detail_nl)`. New route `GET /v1/release-readiness` returns the serialised scorecard with safety booleans hard-False. V1 end-to-end acceptance test runs the morning chain with every leg enabled, asserts each leg returns `succeeded`, and pins the scorecard's `status="ready"` to the same configuration. `docs/deployment.md` gains a "V1 release readiness" section with required env vars, operator runbook (07:00 → manual approval → submit → reconcile) and a V1 scope-lock. `infra/docker/.env.example` consolidates every required V1 flag. `version-1-backlog.md` gains a "V1 is feature-complete" banner; post-V1 widening ideas (full ~5 000-ticker universe, real TimesFM/Chronos clients, conditional orders, GTC/OPG, multi-account portfolios, mobile app, briefing source distinction, persisted per-leg outcomes) are documented but explicitly out of V1 scope.

## Completed slices

- Task 167 completed: deterministic daily briefing + alert digest end-to-end (Slice 12 of the V1 critical path). Migration `0036` adds `daily_briefings` + `briefing_alerts`. Pure-Python `daily_briefing.compute_daily_briefing(...)` returns deterministic counts + Dutch summary + alert list (info / warning / critical). Orchestrator + `POST /briefings/daily/compute` + `GET /briefings/daily/latest`. Portefeuille Dagbriefing panel. Disabled-by-default; no AI authoring; safety booleans hard-False.
- Task 166 completed: Belgian tax module wired into action draft Orderimpact (Slice 11 of the V1 critical path). Pure-Python `belgian_tax` module with locked TOB rates (0.12% / 0.35% / 1.32%) and per-transaction caps + 30% dividend withholding. Migration `0035` adds `estimated_belgian_tob` + `belgian_tob_security_class` to `asset_action_drafts`. Orderimpact and the persisted draft record thread the TOB through; serializer surfaces it; Portefeuille Action drafts table gets a "TOB (BE)" column. Informational only — TOB doesn't change sizing or lift any safety flag.
- Task 165 completed: AI explanation layer (RAG read-only) end-to-end (Slice 10 of the V1 critical path). First AI runtime behind a hard gate. Migration `0034` adds `decision_package_explanations` + `explanation_evidence_ledger` tables. Pure-Python guards in `packages/portfolio` enforce: no hallucinated numbers, locked risk disclaimer required, length + empty checks. `StubExplanationProvider` paraphrases existing Decision Package text deterministically. `generate_explanation(...)` orchestrator. New `POST` + `GET /decision-packages/{id}/explanation` routes. Portefeuille UI subsection per Decision Package. Disabled-by-default; AI never originates a number; safety booleans hard-False on every record and response.
- Task 164 completed: Research Desk runtime wired into the Decision Package evidence chain (Slice 9 of the V1 critical path). Migration `0033` extends `asset_decision_packages` with five research summary columns. Pure-Python `research_evidence_summary` in `packages/portfolio` with deterministic credibility/freshness aggregation and locked blocking reasons. `build_research_summary_by_symbol(...)` helper + threaded `research_summary_by_symbol` kwarg into `sync_decision_packages(...)`. Portefeuille Decision Package card now shows a Research subsection. Read-only context; research evidence can block but never auto-promote.
- Task 163 completed: reconciliation + Prediction Diary end-to-end (Slice 8 of the V1 critical path). `prediction_diary_entries` table (migration 0032) + `PredictionDiaryEntryRecord` + repo + pure-Python `prediction_diary_eval` with deterministic horizon outcome labels. `reconciliation_sync` matches submissions against IBKR open-orders + executions and drives FILLED/CANCELLED → RECONCILED via the locked state machine; `prediction_diary_sync` builds one row per suggestion using the linked forecast + persisted EOD bars. New routes `POST /action-drafts/reconcile`, `POST /prediction-diary/evaluate`, `GET /prediction-diary`. Disabled-by-default; AI never assigns the outcome label; safety booleans hard-False everywhere.
- Task 162 completed: user-approved IBKR paper submission with locked state-machine handshake (Slice 7 of the V1 critical path). `action_draft_state_machine` + new submissions + events tables (migration 0031) + real `IbapiOrderSubmissionClient` + factory + `approve` / `submit-to-ibkr-paper` / `status` routes. Paper-only; disabled-by-default; first slice that places a real order.
- Task 161 completed: action draft skeleton + Orderimpact + dry-run (Slice 6 of the V1 critical path). `action_draft_safety` math in `packages/portfolio`, new `asset_action_drafts` table (migration 0030) + record + repository, `action_draft_sync.generate_action_drafts(...)` orchestrator, `POST /action-drafts/compute` + `GET /action-drafts/latest` + `GET /action-drafts/{draft_id}` routes, Portefeuille Action drafts table. Disabled-by-default; **no submission to IBKR** (that's the next slice).
- Task 160 completed: Decision Package foundation (Slice 5 of the V1 critical path). Immutable, hash-anchored `asset_decision_packages` table (migration 0029) + `AssetDecisionPackageRecord` + insert-only repository, `decision_package_sync` orchestrator that bundles position/cash/market/FX/forecast/suggestion + gate outcomes + audit links, `POST /decision-packages/compute` + `GET /decision-packages/latest` + `GET /decision-packages/{conid}/latest` routes, Portefeuille Decision Packages section. Hard prerequisite gate before any action draft; disabled-by-default; no orders.
