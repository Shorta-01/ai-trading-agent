...
def test_trigger_sell_signal_sweep_targets_correct_path(monkeypatch) -> None:
    """V1.2 §BI — worker cron POST't naar /sell-signals/sweep."""
    ...
    api_trigger.trigger_sell_signal_sweep(
        base_url="http://api:8000", timeout_seconds=1.0
    )
    assert captured["url"] == "http://api:8000/sell-signals/sweep"


def test_trigger_sell_signal_sweep_noops_without_base_url() -> None:
    assert api_trigger.trigger_sell_signal_sweep(
        base_url=None, timeout_seconds=1.0
    ) is None


def test_trigger_sell_signal_sweep_returns_none_on_http_error(
    monkeypatch,
) -> None:
    import httpx
...
# V1.2 §BM / GAPS.md P0-3 — reconciliation sweep goes through the
# in-process ``IbkrReconciler.tick()`` runner, not the api_trigger
# HTTP layer. See ``test_scheduler_init.test_reconciliation_sweep_*``
# for the in-process wiring tests.
