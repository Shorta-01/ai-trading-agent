...
def _with_repository(
    ...
    *,
    ...
    if not storage_settings.enabled or not storage_settings.database_url:
        raise HTTPException(
            status_code=503,
            detail="Opslag is niet verbonden. AssetListing-service is niet beschikbaar.",
        )
    ...
    try:
        with provider.checked_connection(require_writable=require_writable) as checked:
            return operation(
                SqlAlchemyResearchSourceArchiveRepository(checked.connection, checked.readiness)
            )
...
def create_asset_listing(request: AssetListingCreateRequest) -> dict[str, object]:
    def op(repo: SqlAlchemyResearchSourceArchiveRepository) -> dict[str, object]:
        ...
        record = AssetListingRecord(
            ...
            safe_to_use_for_analysis=False,
            safe_to_use_for_suggestions=False,
            blocks_market_data=True,
            blocks_analysis=True,
            blocks_suggestions=True,
            explanation_nl=(
                "Dit is alleen identiteit/referentie-informatie voor een listing, "
                "zonder runtime activering."
            ),
        )
        ...
        return {
            "record": saved,
            "help_nl": (
                "Alleen identiteit/referentie. Geen market-data runtime, analyse, "
                "suggesties, acties of orders."
            ),
            "audit_help_nl": "Gebruik deze basis voor auditbare contract-identiteit.",
...
def list_asset_listings(asset_id: str | None = None) -> dict[str, object]:
    return _with_repository(
        lambda repo: {
            "records": repo.list_asset_listings(asset_id=asset_id),
            "help_nl": "Identity/reference-only overzicht.",
        },
...
def search_asset_listings(q: str = "") -> dict[str, object]:
    return _with_repository(
        lambda repo: {
            "records": repo.search_asset_listings(q, limit=20),
            "help_nl": "Alleen lokale identiteit-zoekactie; geen externe fetch.",
        },
        require_writable=False,
    )
