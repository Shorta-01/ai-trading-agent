...
def fake_storage(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        asset_listings.settings,
        "storage",
        StorageSettings(enabled=True, database_url="postgresql://u:p@db/app"),
    )
            ...
            yield type(
                ...
                (),
                {
                    ...
                    "readiness": MigrationReadinessReport(
                        ...
                        latest_expected_revision_id="0022_asset_listing_identity_foundation",
    ...
    class FakeRepo:
        ...
        def get_asset_by_asset_id(self, asset_id: str):
            return self._assets.get(asset_id)

        def save_asset_listing(self, record: AssetListingRecord):
            self._listings[record.listing_id] = record
            return record
        ...
        def get_asset_listing_by_listing_id(self, listing_id: str):
            return self._listings.get(listing_id)

        def search_asset_listings(self, query: str, limit: int = 20):
            return self.list_asset_listings()[:limit]
