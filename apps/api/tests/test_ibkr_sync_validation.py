...
def _valid_payloads():
    return (
        ...
        [IbkrOpenOrder("paper", 1, None, None, None, "MSFT", "STK", "USD", None, None, "BUY", "LMT", Decimal("1"), Decimal("10"), None, "DAY", "Submitted", Decimal("0"), Decimal("1"), None, datetime.now(UTC), None)],
        [IbkrExecution("paper", "E1", 1, None, "MSFT", "STK", "USD", None, None, "BOT", Decimal("1"), Decimal("10"), datetime.now(UTC), None, None, None, None)],
    )
...
def test_invalid_open_order_payload_fails() -> None:
    cash, positions, orders, execs = _valid_payloads()
    orders[0] = IbkrOpenOrder("paper", -1, None, None, None, "", "STK", "usd", None, None, "HOLD", "", Decimal("0"), Decimal("0"), None, "DAY", "", Decimal("-1"), Decimal("-1"), None, datetime.now(UTC), None)
    result = validate_ibkr_sync_payloads(cash, positions, orders, execs)
    assert result.passed is False
...
def test_invalid_execution_payload_fails() -> None:
    cash, positions, orders, execs = _valid_payloads()
    execs[0] = IbkrExecution("", "", 0, None, "", "STK", "usd", None, None, "X", Decimal("0"), Decimal("0"), "bad", None, None, None, None)  # type: ignore[arg-type]
    result = validate_ibkr_sync_payloads(cash, positions, orders, execs)
    assert result.passed is False
