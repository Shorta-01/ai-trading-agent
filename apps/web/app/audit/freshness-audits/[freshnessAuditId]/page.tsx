...
export default function Page() {
  ...
  const [record, setRecord] = useState<FreshnessDetail | null>(null);
  const [state, setState] = useState<"loading" | "ok" | "not_found" | "error">("loading");
  useEffect(() => { void (async () => { const r = await apiClient.getRequestAuditFreshnessAudit(params.freshnessAuditId); if (!r.ok) return setState("error"); setRecord(r.data as FreshnessDetail); setState("ok"); })(); }, [params.freshnessAuditId]);
