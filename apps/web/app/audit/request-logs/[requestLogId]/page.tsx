...
export default function Page() {
  ...
  const query = useQuery({
    queryKey: ["audit-request-log", params.requestLogId],
    enabled: Boolean(params.requestLogId),
    queryFn: async (): Promise<RequestLogDetail> => {
      const r = await apiClient.getRequestAuditRequestLog(params.requestLogId);
      if (!r.ok) throw new Error("error");
      return r.data as RequestLogDetail;
    },
  });
  const record = query.data ?? null;
  if (query.isLoading) return <main className="page-wrap"><p className="empty-state">Laden...</p></main>;
  if (query.isError || !record) return <main className="page-wrap"><p className="empty-state">API niet bereikbaar.</p><Link href="/audit">← Terug naar auditoverzicht</Link></main>;
