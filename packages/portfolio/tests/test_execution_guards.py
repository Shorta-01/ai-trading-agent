...
def test_execution_guards():
    targets=get_default_execution_targets(); assert set(targets.keys())==set(ExecutionMode)
    ...
    assert check_execution_mode_available(ExecutionMode.IBKR_PAPER, settings.model_copy(update={'allow_ibkr_paper':True}))
    ...
    with pytest.raises(InvalidAccountingInputError): require_execution_mode_available(ExecutionMode.BLOCKED_AUTO, settings)
    bad=targets[ExecutionMode.IBKR_PAPER].model_copy(update={'approval_requirement':ApprovalRequirement.NOT_APPLICABLE})
    with pytest.raises(InvalidAccountingInputError): require_manual_approval_required(bad)
