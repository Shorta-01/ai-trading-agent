...
def get_default_execution_targets() -> dict[ExecutionMode, ExecutionTarget]:
    return {
        ExecutionMode.INTERNAL_PAPER: ExecutionTarget(
            execution_target_id="target_internal_paper",
            ...
            explanation_nl="Interne paper simulator met verplichte goedkeuring.",
        ),
        ExecutionMode.IBKR_PAPER: ExecutionTarget(
            execution_target_id="target_ibkr_paper",
            mode=ExecutionMode.IBKR_PAPER,
            kind=ExecutionTargetKind.IBKR_PAPER_ACCOUNT,
            provider=BrokerProvider.INTERACTIVE_BROKERS,
            account_mode=BrokerAccountMode.IBKR_PAPER,
            status=ExecutionModeStatus.REQUIRES_SETUP,
            approval_requirement=ApprovalRequirement.ALWAYS_REQUIRED,
            can_submit_orders=True,
            can_submit_real_money_orders=False,
            can_read_account_data=True,
            can_read_market_data=True,
            explanation_nl="IBKR paperrekening als toekomstig doel met verplichte goedkeuring.",
        ),
        ExecutionMode.IBKR_LIVE_READ_ONLY: ExecutionTarget(
            execution_target_id="target_ibkr_live_read_only",
            mode=ExecutionMode.IBKR_LIVE_READ_ONLY,
            kind=ExecutionTargetKind.IBKR_LIVE_READ_ONLY,
            provider=BrokerProvider.INTERACTIVE_BROKERS,
            account_mode=BrokerAccountMode.IBKR_LIVE,
            status=ExecutionModeStatus.REQUIRES_SETUP,
            approval_requirement=ApprovalRequirement.NOT_APPLICABLE,
            can_submit_orders=False,
            can_submit_real_money_orders=False,
            can_read_account_data=True,
            can_read_market_data=True,
            explanation_nl="Alleen lezen; geen orderplaatsing.",
        ),
        ExecutionMode.IBKR_LIVE_MANUAL: ExecutionTarget(
            execution_target_id="target_ibkr_live_manual",
            mode=ExecutionMode.IBKR_LIVE_MANUAL,
            kind=ExecutionTargetKind.IBKR_LIVE_MANUAL,
            provider=BrokerProvider.INTERACTIVE_BROKERS,
            account_mode=BrokerAccountMode.IBKR_LIVE,
            status=ExecutionModeStatus.REQUIRES_EXPLICIT_ACTIVATION,
            approval_requirement=ApprovalRequirement.ALWAYS_REQUIRED,
            can_submit_orders=True,
            can_submit_real_money_orders=True,
            can_read_account_data=True,
            can_read_market_data=True,
            explanation_nl=(
                "Toekomstige handmatige live modus met expliciete activatie "
                "en verplichte goedkeuring."
            ),
        ),
...
def check_can_submit_order_to_target(target: ExecutionTarget) -> bool:
    return bool(
        target.can_submit_orders
        and target.approval_requirement == ApprovalRequirement.ALWAYS_REQUIRED
    )
...
def require_manual_approval_required(target: ExecutionTarget) -> None:
    if (
        ...
        raise InvalidAccountingInputError(
            "Manual approval is required for order-capable execution targets"
        )
